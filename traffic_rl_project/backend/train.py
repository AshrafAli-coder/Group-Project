...
def train(update_callback=None, stop_flag=None):
    ...
    # Start once to get environment info
    ...
    for episode in range(EPISODES):
        ...
        while traci.simulation.getMinExpectedNumber() > 0 and steps < MAX_STEPS:
            ...
            next_state = np.array(
                vehicle_counts_next + waiting_times_next + [next_phase],
                dtype=np.float32
            )
            ...
            if len(memory) >= BATCH_SIZE:
                ...
                for i in range(BATCH_SIZE):
                    target_val = rewards[i] + GAMMA * torch.max(next_q[i]) * (1 - dones[i])
        ...
        if update_callback:
            update_callback({
                ...
                "waiting_time": float(total_waiting_time / max(steps,1)),
                "queue_length": float(total_queue_length / max(steps,1)),
                "status": "running"
            })
